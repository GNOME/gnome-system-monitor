               ...
               gpointer data)
{
  ...
  GsmOpenFile *openfiles_data;

  openfiles_data = gsm_open_file_new (NULL, openfiles);
                    ...
                    GHashTable *new_maps)
{
  ...
  data = GSM_OPEN_FILE (g_list_model_get_object (G_LIST_MODEL (store), position));
  ...
  while (old_maps)
    {
      ...
      data = GSM_OPEN_FILE (old_maps->data);
